...
public class StructureComponentVillagePathGen extends StructureComponentVillageRoadPiece
{
    ...
    public StructureComponentVillagePathGen(TemplateManager manager, CompoundNBT nbt)
    {
        super(GCFeatures.CMOON_VILLAGE_PATH, nbt);
    ...
    {
        super(GCFeatures.CMOON_VILLAGE_PATH, par1ComponentVillageStartPiece, par2);
     ...
     */
    @Override
    public boolean create(IWorld par1World, ChunkGenerator<?> generator, Random par2Random, MutableBoundingBox par3StructureBoundingBox, ChunkPos chunkPos)
    {
        final Block var4 = Blocks.SPRUCE_PLANKS;
        ...
        for (int var5 = this.boundingBox.minX; var5 <= this.boundingBox.maxX; ++var5)
        {
            ...
            {
                BlockPos pos = new BlockPos(var5, par1World.getHeight(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, new BlockPos(var5, 0, var6)).getY() - 1, var6);
                BlockState state = par1World.getBlockState(pos);
                if (par3StructureBoundingBox.isVecInside(new BlockPos(var5, 64, var6)) && (state.getBlock() == GCBlocks.moonTurf || Blocks.AIR == state.getBlock()))
                {
                    final int var7 = par1World.getHeight(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, new BlockPos(var5, 0, var6)).getY() - 1;
                    par1World.setBlockState(new BlockPos(var5, var7, var6), var4.getDefaultState(), 2);
                }
            }
        }
