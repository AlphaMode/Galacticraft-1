...
public class SpinManager
{
    ...
    {
        if (this.oneSSBlock == null || this.worldProvider.getWorld().getBlockState(this.oneSSBlock).getBlock().isAir(this.worldProvider.getWorld().getBlockState(this.oneSSBlock), this.worldProvider.getWorld(), this.oneSSBlock))
        {
            if (baseBlock != null)
            {
        ...
        BlockState bsStart = this.worldProvider.getWorld().getBlockState(this.oneSSBlock);
            ...
            for (BlockVec3 vec : currentLayer)
            {
                ...
                for (int LogicalSide = 0; LogicalSide < 6; LogicalSide++)
                {
                    ...
                    if (sideVec != null && !this.checked.contains(sideVec))
                    {
                        ...
                        BlockState state = sideVec.getBlockState(this.worldProvider.getWorld());
                        if (state == null)
                        {
                        	continue;
                        }
                        Block b = state.getBlock();
                        if (b != null && !b.isAir(state, this.worldProvider.getWorld(), sideVec.toBlockPos()))
                        {
                            nextLayer.add(sideVec);
                            if (bStart.isAir(bsStart, this.worldProvider.getWorld(), this.oneSSBlock))
                            {
                            ...
                            if (b instanceof BlockSpinThruster && !RedstoneUtil.isBlockReceivingRedstone(this.worldProvider.getWorld(), sideVec.toBlockPos()))
                            {
        ...
        if (placingThruster && !this.checked.contains(new BlockVec3(baseBlock)))
        {
            ...
            {
                this.oneSSBlock = baseBlock;
                BlockState state = this.worldProvider.getWorld().getBlockState(this.oneSSBlock);
        ...
        {
            BlockState state = this.worldProvider.getWorld().getBlockState(thruster);
            boolean reversed = state.get(BlockSpinThruster.ORIENTATION);
            if (reversed)
            {
        ...
        GalacticraftCore.packetPipeline.sendToDimension(new PacketSimple(PacketSimple.EnumSimplePacket.C_UPDATE_STATION_DATA, GCCoreUtil.getDimensionType(this.worldProvider), objList), GCCoreUtil.getDimensionType(this.worldProvider));
        ...
        GalacticraftCore.packetPipeline.sendToDimension(new PacketSimple(PacketSimple.EnumSimplePacket.C_UPDATE_STATION_BOX, GCCoreUtil.getDimensionType(this.worldProvider), objList), GCCoreUtil.getDimensionType(this.worldProvider));
    ...
    public void updateSpin()
    {
        if (!this.clientSide)
        {
            ...
            {
                this.savefile = OrbitSpinSaveData.initWorldData((ServerWorld) this.worldProvider.getWorld());
            ...
            if (this.doSpinning)
            {
                ...
                if (updateNeeded)
                {
                    ...
                    GalacticraftCore.packetPipeline.sendToDimension(new PacketSimple(PacketSimple.EnumSimplePacket.C_UPDATE_STATION_SPIN, GCCoreUtil.getDimensionType(this.worldProvider), objList), GCCoreUtil.getDimensionType(this.worldProvider));
                }
            }

            //Update entity positions if in freefall
            this.loadedEntities.clear();
            this.loadedEntities.addAll(((ServerWorld)this.worldProvider.getWorld()).getEntities().collect(Collectors.toList()));
            for (Entity e : this.loadedEntities)
            {
                ...
                {
                    AxisAlignedBB entityBoundingBox = e.getBoundingBox();
                    ...
                    if (outsideStation || FreefallHandler.testEntityFreefall(this.worldProvider.getWorld(), entityBoundingBox))
                    {
                        if (this.doSpinning)
                        {
    ...
    private void moveRotatedEntity(Entity e, double rotationCentreX, double rotationCentreZ, float deltaTheta)
    {
        ...
        if (deltaTheta != 0F)
        {
            ...
            e.setRawPosition(e.getPosX() + offsetX, e.getPosY(), e.getPosZ() + offsetZ);
            ...
            e.setBoundingBox(e.getBoundingBox().offset(offsetX, 0.0D, offsetZ));
            ...
            double velocity = Math.sqrt(e.getMotion().getX() * e.getMotion().getX() + e.getMotion().getZ() * e.getMotion().getZ());
            if (e.getMotion().getX() == 0D)
            {
                angle = e.getMotion().getZ() > 0 ? 3.1415926535F / 2 : -3.1415926535F / 2;
            }
            else
            {
                angle = (float) Math.atan(e.getMotion().getZ() / e.getMotion().getX());
            }
            if (e.getMotion().getX() < 0D)
            {
                angle += 3.1415926535F;
            ...
            e.setMotion(velocity * MathHelper.cos(angle), e.getMotion().getY(), velocity * MathHelper.sin(angle));
        }
    }
...
//            if (p.world.getCollisionBoxes(p, p.getBoundingBox()).size() == 0)
//            {
//                //Now check for collisions in the new direction and if there are some, try reducing the movement
//                int collisions = 0;
//                do
//                {
//                    List<AxisAlignedBB> list = p.world.getCollisionBoxes(p, p.getBoundingBox().expand(offsetX, 0.0D, offsetZ));
...
//                while (collisions > 0);
//
//                p.setRawPosition(p.getPosX() + offsetX, p.getPosY(), p.getPosZ() + offsetZ);
...
//            }
    ...
    public void applyCentrifugalForce(ClientPlayerEntity p)
    {
        ...
        {
        case 0:
            p.setMotion(mot.getX() + accel, mot.getY(), mot.getZ());
            break;
        case 1:
            p.setMotion(mot.getX(), mot.getY(), mot.getZ() + accel);
            break;
        case 2:
            p.setMotion(p.getMotion().subtract(accel, 0, 0));
            break;
        case 3:
        default:
            p.setMotion(p.getMotion().subtract(0, 0, accel));
        }
    }
    ...
    public void sendPackets(ServerPlayerEntity player)
    {
        ...
        {
            GalacticraftCore.packetPipeline.sendTo(new PacketSimple(PacketSimple.EnumSimplePacket.C_UPDATE_STATION_SPIN, GCCoreUtil.getDimensionType(player.world), objList), player);
        }
        ...
        {
            GalacticraftCore.packetPipeline.sendTo(new PacketSimple(PacketSimple.EnumSimplePacket.C_UPDATE_STATION_DATA, GCCoreUtil.getDimensionType(player.world), objList), player);
        }
        ...
        {
            GalacticraftCore.packetPipeline.sendTo(new PacketSimple(PacketSimple.EnumSimplePacket.C_UPDATE_STATION_BOX, GCCoreUtil.getDimensionType(player.world), objList), player);
        }
    }
    ...
    public void readFromNBT(CompoundNBT nbt)
    {
        ...
        CompoundNBT oneBlock = (CompoundNBT) nbt.get("oneBlock");
        if (oneBlock != null)
        {
//            this.oneSSBlock = BlockVec3.readFromNBT(oneBlock);
            this.oneSSBlock = new BlockPos(oneBlock.getInt("x"), oneBlock.getInt("y"), oneBlock.getInt("z"));
        }
        else
        {
    ...
    public CompoundNBT writeToNBT(CompoundNBT nbt)
    {
        ...
        if (this.oneSSBlock != null)
        {
            ...
            oneBlock.putInt("x", this.oneSSBlock.getX());
            oneBlock.putInt("y", this.oneSSBlock.getY());
            oneBlock.putInt("z", this.oneSSBlock.getZ());
            nbt.put("oneBlock", oneBlock);
        }
        return nbt;
    }
}
